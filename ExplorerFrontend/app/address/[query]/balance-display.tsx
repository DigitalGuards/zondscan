...
export default function BalanceDisplay({ balance }: BalanceDisplayProps): JSX.Element {
  ...
  return (
    <div className="relative overflow-hidden rounded-lg md:rounded-xl 
      ...
      <h2 className="text-xs md:text-sm font-semibold text-text-secondary mb-2 md:mb-3 lg:mb-4">Balance</h2>
      <div className="flex items-baseline flex-wrap gap-1.5 md:gap-2">
        <span className="text-lg md:section-title break-all">{formattedBalance}</span>
        <span className="text-[10px] md:text-xs lg:text-sm text-text-secondary">{unit}</span>
      </div>
