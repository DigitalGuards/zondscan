...
export default function RichlistClient({ richlist }: RichlistProps) {
  ...
  const renderMobileView = () => (
    <div className="space-y-4">
      {richlist.map((item: any, index: number) => (
        <div key={item.id} className="p-4 rounded-lg border border-[#3d3d3d] bg-gradient-to-br from-[#2d2d2d] to-[#1f1f1f]">
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <span className="ml-2 text-white text-sm">{toFixed(item.balance)} QRL</span>
            </div>
          </div>
        </div>
      ))}
    ...
    <div className="min-h-screen">
      <div className="max-w-[1200px] mx-auto p-4 md:p-8">
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-sm md:text-base text-gray-400 mt-2">Top 50 QRL holders by balance</p>
        </div>
